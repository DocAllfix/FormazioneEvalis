...
export async function recordHeartbeat(params: {
  ...
  if (existing) {
    await db
      ...
      .set({ effectiveSeconds, audioCompleted, completedAt, updatedAt: now })
      .where(eq(slideProgress.id, existing.id));
  } else {
    await db.insert(slideProgress).values({ enrollmentId, slideId, effectiveSeconds, audioCompleted, completedAt, updatedAt: now });
  }
